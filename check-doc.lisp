...
(defun do-documented-names% (thunk xml-pathname)
  (labels ((find-documentation (node)
    ...
    (find-documentation (cxml:parse-file xml-pathname (cxml-xmls:make-xmls-builder))))
  nil)
...
(defun documented-names (xml-pathname)
  "Returns a list of strings, the symbols that are documented in index.xml"
  (let (names)
    (do-documented-names (symbol-name xml-pathname)
...
(defun check-function-argument-documentation (package xml-pathname)
  (do-documented-names (symbol-name xml-pathname symbol-type node)
    (when (find symbol-type '("function" "reader" "accessor") :test #'equal)
      (handler-case
          (fdefinition (find-symbol symbol-name package))
        (error (e)
          (declare (ignore e))
          (return)))
      (let* ((real-arglist (cleanup-arglist (swank::arglist (find-symbol symbol-name package))))
...
(defun dump-docstring-and-description (package xml-pathname)
  (do-documented-names (symbol-name xml-pathname symbol-type node)
    (when (find symbol-type '("function" "accessor") :test #'equal)
      (let ((docstring (clean-string (documentation (find-symbol symbol-name package) 'function)))
...
(defun exported-names (package)
  "Return list of strings, the symbols that are exported from PACKAGE"
  (let (names)
    (do-external-symbols (symbol package)
      (pushnew (symbol-name symbol) names))
    names))

(defun check-doc (package xml-pathname)
  (format t "---------------------~%")
  (let* ((documented (documented-names xml-pathname))
         (exported (exported-names package))
  ...
  (check-function-argument-documentation package xml-pathname))
